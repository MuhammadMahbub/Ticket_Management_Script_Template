    ...
    <div class="user_list user-page table-responsive table-overflow-none mt-2">
        <div class="mb-2">
            ...
            @foreach ($all_langs as $item)
                <a href="{{ route('language_wise_data', $item->short_name) }}">
                    <button data="{{ $item->short_name }}" lang_name="{{ $item->name }}" class="country_short_name p-2 mt-3 mb-3 btn btn-sm btn-info {{ $basename == $item->short_name ? 'active' : '' }}">
                        {{ $item->name }}
                    </button>
                </a>
            @endforeach
